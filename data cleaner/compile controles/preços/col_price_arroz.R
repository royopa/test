...
for (i in 1:n_yrs){
  ...
  arch <- list.files(path = path0, pattern = "rroz")
...
### Dados faltantes: uf = 53 para os anos 08, 14 e 15
c1 <- c(53, 2008, base$price[base$uf == 52 & base$ano == 2008])
base <- rbind(base, c1)
c1 <- c(53, 2014, base$price[base$uf == 52 & base$ano == 2014])
base <- rbind(base, c1)
c1 <- c(53, 2015, base$price[base$uf == 52 & base$ano == 2015])
base <- rbind(base, c1)

write.csv(base, paste0(dataplace, "base_arroz.csv"), row.names=FALSE)
