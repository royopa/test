...
rain <- read.csv(file ="DADOS/04.Controles/base_rain.csv", header=TRUE, sep=",")

### Merging datasets
base <- merge(pbf, emp, by=c("cod_mun","ano"))
base <- merge(base, rain, by=c("cod_mun","ano"))
...
write.csv(base, "BASES/Rain_base.csv", row.names=FALSE)
#write.dta(base, "STATA/Rain_base.dta")