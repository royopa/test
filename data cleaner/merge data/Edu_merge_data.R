...
nat <- read.csv(file ="DADOS/04.Controles/base_nat.csv", header=TRUE, sep=",")

### Merging datasets
base <- merge(pbf, emp, by=c("cod_mun","ano"))
base <- merge(base, nat, by=c("cod_mun","ano"))
...
write.csv(base, "BASES/Edu_base.csv", row.names=FALSE)
#write.dta(base, "STATA/Edu_base.dta")