...
tse <- read.csv(file ="DADOS/04.Controles/base_tse_bey.csv", header=TRUE, sep=",")

### Merging datasets
base <- merge(pbf, emp, by=c("cod_mun","ano"))
base <- merge(base, tse, by=c("cod_mun","ano"))
...
write.csv(base, "BASES/TSE_base_bey.csv", row.names=FALSE)
#write.dta(base, "STATA/TSE_base.dta")