...
mort <- read.csv(file ="DADOS/04.Controles/base_mort.csv", header=TRUE, sep=",")

### Merging datasets
base <- merge(pbf, emp, by=c("cod_mun","ano"))
base <- merge(base, mort, by=c("cod_mun","ano"))
...
write.csv(base, "BASES/Mort_base.csv", row.names=FALSE)
#write.dta(base, "STATA/Mort_base.dta")