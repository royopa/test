...
horizonte_max <- 55
itens_monitor <- c(1101002, 1102, 1103003, 1103028, 1103043, 1103044, 1106003, 
                   1106008, 1106039, 1106018, 1106028, 1107018, 1107084, 1107087, 1107088, 1107089, 
                   1107093, 1107094, 1107095, 1107099, 1110009, 1110010, 1110044, 1111004)#1101073, 

itens_atacado <- c("arroz", "trigo", "batata", "tomate", "cebola", "cenoura", "abacaxi", 
                   "banana", "laranja", "mamao", "uva", "suinos", "boi", "boi", "boi", "boi", "boi", "boi",
                   "boi", "boi", "frango", "frango", "ovo", "leite")#"feijao", 

itens_cepea <- c("arroz", "trigo", "suinos", "boi", "frango", "leite")

for (k in 1:length(itens_monitor)){
  it_m <- itens_monitor[k]
  it_a <- itens_atacado[k]
  dfY <- readbase_monitor(itens=it_m, media=FALSE, end=FALSE, last=FALSE, tri=FALSE, 
                            initial.date="2012-01-01")
  
  if (it_a %in% itens_cepea){ 
    dfX_all <- readbase_cepea(prods=it_a, initial.date="2012-01-01", mm.days=7, varp30=TRUE)
...
write_xlsx(optimal, 
           "F:/DADOS/ASSET/MACROECONOMIA/1PIETRO/AUXILIARES/rotinas juliano/prev_monitfgv_ponta_atac.xlsx", 
           col_names = TRUE)
