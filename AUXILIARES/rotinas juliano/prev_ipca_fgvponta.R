...
for (k in 1:length(itens)){
  ...
  dfY     <- readbase_ipca(itens=it_m, day=28, var=TRUE, tri=FALSE, initial.date = "2012-01-31")
  dfX_all <- readbase_monitor(itens=it_m, media=FALSE, end=FALSE, last=FALSE, 
                              tri=FALSE, initial.date="2012-01-01")
...
write_xlsx(optimal, 
           "F:/DADOS/ASSET/MACROECONOMIA/1PIETRO/AUXILIARES/rotinas juliano/prev_ipca_fgvponta.xlsx", 
           col_names = TRUE)
