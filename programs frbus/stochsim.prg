...
' A list of stochastic equations is part of the information
' loaded by the read_xml_model addin.
...
' successive quarters at a time, etc.
...
' (%threshold parameter).  The threshold conditions should only
' be invoked when the ZLB is binding in the baseline at the
' start of the simulation period.  When the threshold conditions
' are used, the values of the threshold trigger variables -- 
' lurtrsh and pitrsh -- need to be set below to appropriate values.
...
' Monetary policy
  %zerobound  = "yes"
  %threshold  = "no"  'see introductory comment about the "yes" option
  %rff_weight_q1  = ".50"
...
'         the baseline funds rate (rff) is within 25 basis points of the
'         zero bound variable (rffmin).

  smpl %simstart %simend
  series not_constrnd = ((rff - rffmin) >= .25)
...
' copy historical residuals into series whose names have _err suffixes
 ...
 '*********************************************
' use m_stoch string to create group of equations to receive shocks
  %tmp = " "
