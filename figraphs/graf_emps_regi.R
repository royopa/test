...
q <- 4
quebra_emp <- c("horsex", "tamsex", "remsex", "setsex")
qe <- quebra_emp[q]
levels <- c("N","NE","SE","S","CO")

###### END EDIT

base <- read.csv(file =paste0("DADOS/base_emp_",qe,".csv"), header =TRUE, sep =",")
nvar <- dim(base)[2]
base$fac_div <- base$re <- trunc(base$cod_mun/100000)
...
reg <- ggplot(base0, aes(x = ano, y = value, color = variable)) +
    ...
    ylab("Emprego") +
    xlab("Anos")
ggsave(paste0("FigGrafs/Quebra nas Colunas/Grafs_regi/regi_",qe,".png"), scale=2)
