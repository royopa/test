...
qe <- quebra_emp[q]
levels <- c("1st Quint Y","2nd Quint Y","3rd Quint Y","4th Quint Y","5th Quint Y")
...
nvar <- dim(base)[2]
base11 <- base[,1:2]
base11$emp_tot <- rowSums(base[,3:nvar])
cont <- read.csv(file ="Dados/base_pop.csv", header =TRUE, sep =",")
base11 <- merge(base11, cont, by=c("cod_mun","ano"))
base00 <- base11 %>% group_by(cod_mun) %>% select(emp_tot,pop) %>% summarise(emp_med=mean(emp_tot/pop))
base <- merge(base, base00, by=c("cod_mun"))
base$fac_div <- quantcut(base$emp_med, q=5, na.rm=TRUE)
...
reg <- ggplot(base0, aes(x = ano, y = value, color = variable)) +
    ...
    ylab("Emprego") +
    xlab("Anos")
ggsave(paste0("FigGrafs/Quebra nas Colunas/Grafs_varY/varY_",qe,".png"), scale=2)
