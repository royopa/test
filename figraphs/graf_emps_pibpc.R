...
qe <- quebra_emp[q]
levels <- c("1st Quint PIBPC","2nd Quint PIBPC","3rd Quint PIBPC","4th Quint PIBPC","5th Quint PIBPC")

###### END EDIT

base <- read.csv(file =paste0("Dados/base_emp_",qe,".csv"), header =TRUE, sep =",")
nvar <- dim(base)[2]
cont <- read.csv(file ="Dados/pib10.csv", header =TRUE, sep =";", dec=",")
cont$fac_div <- cont$fac_pib <- quantcut(cont$pibpc2010, q=5, na.rm=TRUE)
cont <- cont[,c("cod_mun","fac_div")]
base <- merge(base, cont, by=c("cod_mun"))
...
reg <- ggplot(base0, aes(x = ano, y = value, color = variable)) +
    ...
    ylab("Emprego") +
    xlab("Anos")
ggsave(paste0("FigGrafs/Quebra nas Colunas/Grafs_pibpc/pibpc_",qe,".png"), scale=2)
